...
parser.add_argument('--tag', type=str, help="experiment ID")
...
class MSDNETConfig():
    ...
    if depth == 'm' or depth == 'l':
        ...
        reduction=0.5
        resume=False
        save='./model_files/{}_{}'.format(tag,time.strftime('%Y.%m.%d_%H.%M.%S')) # change the save path to a new path
        seed=0
        start_epoch=0
        step=2
        stepmode='even' 
...
def run_training():
    ...
    train_loader, val_loader, test_loader = get_train_dataloaders(args)

...
for color in ['gray', 'color']:
    ...
    if color == 'gray':
        model.load_weights(args.load_path_gray,skip_mismatch=True,by_name=True)
    else:
        model.load_weights(args.load_path_color,skip_mismatch=True,by_name=True)
    ...
    color_results[args.color] = [sum(np.argmax(predictions[i],axis=1)==np.argmax(testy[i],axis=1))/len(predictions[i]) for i in range(8)]
